    ...
    <section className="bg-[#F6F7EE] py-14 md:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          {posts.map((p) => (
            <article key={p.title} className="overflow-hidden bg-white shadow-sm">
              {/* Image only has rounded corners */}
              ...
              {/* Text section (no border radius added) */}
              <div className="p-5 text-green-900">
                <div className="mt-2 flex items-center gap-2 text-neutral-500 text-xs">
                  ...
                  <span>{p.date}</span>
                  <span>•</span>
                  <img src="/profile.png" alt="profile" className="h-3 w-3 object-contain rounded-full" />
                  <span>{p.author}</span>
                </div>
